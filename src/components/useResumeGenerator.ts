...
import type { MasterProfile, ParseResult } from "@/lib/types";
...
export function useResumeGenerator() {
  ...
  const generate = useCallback(async (text: string) => {
    ...
    // Open preview window immediately to avoid popup blocking
...
<body>
  <div class="card">
    ...
    try {
      ...
      if (data.debugError) {
        console.warn("AI parse fallback triggered:", data.debugError);
      }

      // Write the generated page into the preview window
      renderGeneratedPage(preview, data.profile);
    } catch (err) {
      preview.close();
...
function renderGeneratedPage(preview: Window, profile: MasterProfile) {
  // The generated page HTML is built by the GeneratedPage component equivalent.
  // For now, we write the structured profile as a data attribute and let a
  // minimal client-side script render it.
  // Full implementation in GeneratedPage component which will be ported next.

  preview.document.open();
  preview.document.write(buildGeneratedPageHtml(profile));
  preview.document.close();
}

// ---- Page HTML builder (ported from original index.html buildGeneratedPageHtml) ----
...
function renderBullets(items: string[]): string {
  ...
  return `<ul class="group/list">${items.map((item) => `<li contenteditable="true" style="outline:none" onblur="if(!this.textContent.trim()&&this.parentNode.children.length>2)this.remove()">${escapeHtml(item)}</li>`).join("")}<div class="opacity-0 group-hover/list:opacity-100 transition mt-2 cursor-pointer text-blue-500 text-xs font-medium inline-flex items-center gap-1 select-none" onclick="var li=document.createElement('li');li.contentEditable='true';li.style.outline='none';li.setAttribute('onblur','if(!this.textContent.trim()&&this.parentNode.children.length>2)this.remove()');this.parentNode.insertBefore(li,this);li.focus();"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> 添加一行</div></ul>`;
}

function buildGeneratedPageHtml(profile: MasterProfile): string {
      ...
      .map(
        (tag, i) =>
          `<span class="inline-block px-4 py-2 rounded-full text-xs font-medium shadow-sm ${
            ["bg-blue-600 text-white", "bg-blue-500 text-white", "bg-blue-400 text-white", "bg-blue-300 text-blue-900", "bg-blue-200 text-blue-900", "bg-blue-100 text-blue-900", "bg-gray-100 text-gray-700"][i % 7]
          }">${escapeHtml(tag)}</span>`
      )
      .join("") || "";
    ...
    (profile.projects || [])
      ...
      <article class="rounded-[1.75rem] border border-gray-100 bg-white p-6 shadow-sm">
        <div class="aspect-[16/10] rounded-[1.25rem] overflow-hidden bg-gray-100 mb-5">
          <img src="https://images.unsplash.com/photo-${["1587614382346-4ec70e388b28", "1511578314322-379afb476865", "1520607162513-77705c0f0d4a", "1516321318423-f06f85e504b3"][i % 4]}?auto=format&fit=crop&q=80&w=800" class="w-full h-full object-cover" alt="项目配图">
        </div>
